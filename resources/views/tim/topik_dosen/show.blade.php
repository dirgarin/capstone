                                    ...
                                    <div class="ms-auto d-flex gap-2">
                                        ...
                                        <form action="{{ route('topik_dosen.update', $data->id) }}" method="post">
                                            ...
                                            <button type="submit" class="btn btn-danger"
                                                onclick="return confirmMessage('Apakah anda yakin untuk menolak topik dosen ini?', event)">
                                                <i class="fas fa-times"></i> Tolak
                                            </button>
                                        </form>
                                    </div>
                                </div>
                            @elseif($data->daftar_topik_dosen->where('status', 'assigned')->count() >= $data->jumlah_tim)
                                @if($data->daftar_topik_dosen->first()->daftar_topik->where('status', 'approved')->count() < 3)
                                    <div class="col-12 d-flex">
                                        <div class="ms-auto d-flex gap-2">
                                            <form action="{{ route('topik_dosen.update', $data->id) }}" method="post">
                                                @csrf
                                                ...
                                                <button type="submit" class="btn btn-success"
                                                    onclick="return confirmMessage('Apakah anda yakin untuk approve topik dosen ini dengan 3 tim terpilih?', event)">
                                            ...
                                            <form action="{{ route('topik_dosen.update', $data->id) }}" method="post">
                                                ...
                                                <input type="hidden" name="status" value="rejected">
                                                <button type="submit" class="btn btn-danger"
                                                    onclick="return confirmMessage('Apakah anda yakin untuk menolak topik dosen ini dengan 3 tim terpilih?', event)">
